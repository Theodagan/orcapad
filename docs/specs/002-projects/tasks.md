...
- [ ] **PROJ-T1 — Ports**
  ...
  `mobile/src/core/application/ports/`.

  **Verify:** `pnpm typecheck`; layering test green.

- [ ] **PROJ-T2 — Project mapping and adapter**

  `src/adapters/orca/rpc/repo-operations.ts` (`repo.list`, `repo.show`) and
  `src/adapters/orca/mapping/project-mapping.ts`.

  **Verify:** `pnpm test src/adapters/orca/mapping/project-mapping.test.ts`

- [ ] **PROJ-T3 — Workspace mapping**

  `src/adapters/orca/mapping/workspace-mapping.ts` implementing `tech.md` §2.1,
  with the adapter-side extras (`path`, `terminalPlatform`, `orderHint`) held in
  a separate record, not on the domain type.

  **Verify:** `pnpm test src/adapters/orca/mapping/workspace-mapping.test.ts`

- [ ] **PROJ-T4 — Attention projection**

  `src/adapters/orca/mapping/workspace-attention-projection.ts` per §2.2, plus a
  ...
  **Verify:** `pnpm test src/adapters/orca/mapping/workspace-attention-projection.test.ts`

- [ ] **PROJ-T5 — Review link projection**

  `src/adapters/orca/mapping/review-link-projection.ts` per §2.3. Provider-neutral
  naming; no `github` identifier in the domain or the component names.

  **Verify:** `pnpm test src/adapters/orca/mapping/review-link-projection.test.ts`

- [ ] **PROJ-T6 — Conditional-refresh workspace adapter**

  `src/adapters/orca/rpc/worktree-operations.ts` (`worktree.ps`, `worktree.show`,
  `worktree.activate`) and `src/adapters/orca/workspace-adapter.ts` implementing
  `WorkspacePort` with the `unchanged` path.

  **Verify:** `pnpm test src/adapters/orca/workspace-adapter.test.ts` (PROJ-AC5)

- [ ] **PROJ-T7 — Cross-host list use case**

  `src/core/application/use-cases/list-controller-workspaces.ts` — merge per
  connection, stamp the connection, never deduplicate.

  **Verify:** `pnpm test src/core/application/use-cases` (PROJ-AC7)
...
- [ ] **PROJ-T8 — Filters and ordering state**
  ...
  `src/features/projects/state/`. Persist filters through the existing
  preferences store.

  **Verify:** `pnpm test src/features/projects/state` (PROJ-AC3, AC6)
...
- [ ] **PROJ-T10 — Workspace detail screen**
  ...
  **Verify:** `pnpm test src/features/projects/screens`
