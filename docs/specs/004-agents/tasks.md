...
- [ ] **AG-T1 — Port**
  ...
  `src/gamepad/application/`.

  **Verify:** `pnpm typecheck`; layering test green.

- [ ] **AG-T2 — Journal item mapping**

  `src/gamepad/adapters/orca/mapping/journal-item-mapping.ts` covering `message`,
  `toolCall`, `diff`, `approval`, `question`, and the `turn` / `status`
  diversions.

  **Verify:** `pnpm test src/gamepad/adapters/orca/mapping/journal-item-mapping.test.ts`

- [ ] **AG-T3 — Mutation envelope**

  `src/gamepad/adapters/orca/rpc/agent-mutation-envelope.ts` — fingerprinting with
...
- [ ] **AG-T4 — History operation and paging**
  ...
  `src/gamepad/adapters/orca/rpc/agent-history-operation.ts` with direction translation
...
- [ ] **AG-T5 — Subscribe operation and fence gate**
  ...
  `src/gamepad/adapters/orca/rpc/agent-subscribe-operation.ts` mapping the four wire
...
- [ ] **AG-T6 — Mutating operations**
  ...
  **Verify:** `pnpm test src/gamepad/adapters/orca/rpc/agent-mutation-operations.test.ts`

- [ ] **AG-T7 — Transcript reducer**

  `src/gamepad/features/agents/state/transcript-reducer.ts` with the five invariants.

  **Verify:** `pnpm test src/gamepad/features/agents/state/transcript-reducer.test.ts`

- [ ] **AG-T8 — Pending-mutation registry**

  `src/gamepad/features/agents/state/pending-mutation-registry.ts` over
...
- [ ] **AG-T10 — Transcript rendering**
  ...
  **Verify:** `pnpm run check:code-quality:changed`;
  `pnpm test src/gamepad/features/agents` (AG-AC1)
...
- [ ] **AG-T15 — Screen assembly**
  ...
  **Verify:** `pnpm test src/gamepad/features/agents/screens`; CDP screenshot check per
